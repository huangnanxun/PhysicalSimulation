...
void add_wind(){
  ...
  for (int i = 0; i < numD; i++) {
    ...
    for (int j = 1; j < numV; j++) {
      //float nois = noise(x_o, y_o);
      //particles[i][j].display();
      float wind_x = map(noise(x_o, y_o, z_o), 0, 1, 0, 3);
      float wind_y = map(noise(x_o+40, y_o+40, z_o), 0, 1, -0.5, 0); ///500 500 0
      float wind_z = map(noise(x_o+90, y_o+90, z_o), 0, 1, -1, 1);///300 300 0
      posX[j] += wind_x;
      posY[j] += wind_y;
      y_o += 0.1;
    }
